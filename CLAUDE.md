...
浏览器插件无需打包即可加载：**`apps/extension`**（构建后用 **`apps/extension/dist`**）。
...
| 看最近改了什么 | [docs/current-status.md](docs/current-status.md)（82KB 流水账） |
...
- **README.md 严重过时**（仍是 5/11 原型描述）—— 任何新成员以本文件 + `docs/operating-principles.md` + `docs/roadmap-to-production.md` 为准。

## 10. Git 现状

- 仓库已初始化（`master` 分支），首提交 `4de04c5 chore: import existing monorepo as baseline`（97 files / 42732 insertions）。
- 暂未配 remote，纯本地。配远程：`git remote add origin <url>` → `git push -u origin master`。
